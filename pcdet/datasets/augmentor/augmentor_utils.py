...
def scale_pre_object(gt_boxes, points, gt_boxes_mask, scale_perturb, num_try=50):
    ...
    scale_noises = np.random.uniform(
        scale_perturb[0], scale_perturb[1], size=[num_boxes, num_try]
    )
    for k in range(num_boxes):
        ...
        scl_box[:, 3:6] = scl_box[:, 3:6] * scale_noises[k].reshape(-1, 1).repeat(
            [3], axis=1
        )
        ...
        point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(
            points[:, 0:3], np.expand_dims(gt_boxes[k], axis=0)
        ).squeeze(0)
        ...
        obj_points = common_utils.rotate_points_along_z(
            np.expand_dims(obj_points, axis=0), -ry
        ).squeeze(0)
        ...
        if scale_noises[k][try_idx] > 1:
            points_dst_mask = roiaware_pool3d_utils.points_in_boxes_cpu(
                points[:, 0:3], np.expand_dims(gt_boxes[k], axis=0)
            ).squeeze(0)
...
def normalize_object_size(boxes, points, boxes_mask, size_res):
    ...
    for k in range(boxes.shape[0]):
        ...
        if (np.array(size_res) > 0).any():
            points_dst_mask = roiaware_pool3d_utils.points_in_boxes_cpu(
                points[:, 0:3], np.expand_dims(boxes[k], axis=0)
            ).squeeze(0)
...
def rotate_objects(gt_boxes, points, gt_boxes_mask, rotation_perturb, prob, num_try=50):
    ...
    for idx in range(num_boxes):
        ...
        point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(
            points[:, 0:3], np.expand_dims(gt_boxes[idx], axis=0)
        ).squeeze(0)
        ...
        object_points = common_utils.rotate_points_along_z(
            object_points[np.newaxis, :, :], np.array([rot_noise[idx][try_idx]])
        )[0]
        ...
        points_dst_mask = roiaware_pool3d_utils.points_in_boxes_cpu(
            points[:, 0:3], np.expand_dims(rot_box[try_idx], axis=0)
        ).squeeze(0)
