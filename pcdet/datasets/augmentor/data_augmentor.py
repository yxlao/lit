...
class DataAugmentor(object):
    ...
    def normalize_object_size(self, data_dict=None, config=None):
        ...
        # Backup unscaled points and gt_boxes
        ...
        visualize = False
        if visualize:
            boxes_mask = data_dict["gt_boxes_mask"]

            src_pcd = o3d.geometry.PointCloud()
            src_pcd.points = o3d.utility.Vector3dVector(src_points[:, :3])
            src_pcd.paint_uniform_color([0.0, 0.0, 1.0])
            src_bboxes_ls = bboxes_to_lineset(
                src_gt_boxes[boxes_mask], frame_pose=np.eye(4)
            )
            src_bboxes_ls.paint_uniform_color([0.0, 0.0, 1.0])

            dst_pcd = o3d.geometry.PointCloud()
            dst_pcd.points = o3d.utility.Vector3dVector(points[:, :3])
            dst_pcd.paint_uniform_color([1.0, 0.0, 0.0])
            dst_bboxes_ls = bboxes_to_lineset(
                gt_boxes[boxes_mask], frame_pose=np.eye(4)
            )
            dst_bboxes_ls.paint_uniform_color([1.0, 0.0, 0.0])
