...
def filter_by_range(
    infos, gt_key, range_min=0, range_max=80, is_pred=False, dataset="kitti"
):
...
def transform_to_kitti_format(pred_infos, gt_annos, dataset, fakelidar):
    ...
    kitti_utils.transform_annotations_to_kitti_format(
        pred_infos, map_name_to_kitti=map_name_to_kitti
    )
    kitti_utils.transform_annotations_to_kitti_format(
        gt_annos,
        map_name_to_kitti=map_name_to_kitti,
        info_with_fakelidar=fakelidar,
        **kwargs,
    )
...
def main():
    ...
    for cur_range in range_list:
        cur_pred_info, num_pred_objs = filter_by_range(
            pred_infos,
            gt_keys[args.dataset],
            range_min=cur_range[0],
            range_max=cur_range[1],
            is_pred=True,
        ...
        print(
            f"----------Range={cur_range}, avg_pred_objs={num_pred_objs / len(pred_infos)}, "
