...
class KittiDataset(DatasetTemplate):
    def __init__(
        ...
        split_dir = (
            self.root_path.parent.parent / "data_split" / "kitti" / f"{self.split}.txt"
        )
        self.sample_id_list = (
    ...
    def include_kitti_data(self, mode):
        ...
        if self.logger is not None:
            self.logger.info(
                "Total samples for KITTI dataset: %d" % (len(self.kitti_infos))
            )
    ...
    def get_fov_flag(pts_rect, img_shape, calib, margin=0):
        ...
        val_flag_1 = np.logical_and(
            pts_img[:, 0] >= 0 - margin, pts_img[:, 0] < img_shape[1] + margin
        )
        val_flag_2 = np.logical_and(
            pts_img[:, 1] >= 0 - margin, pts_img[:, 1] < img_shape[0] + margin
        )
    ...
    def generate_prediction_dicts(
        ...
        def generate_single_sample_dict(batch_index, box_dict, force_no_filter=False):
            ...
            # BOX FILTER
            if (
                self.dataset_cfg.get("TEST", None)
                and self.dataset_cfg.TEST.BOX_FILTER["FOV_FILTER"]
                and not force_no_filter
            ):
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            single_pred_dict = generate_single_sample_dict(
                index, box_dict, force_no_filter=force_no_filter
            )
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        save_all_eval_results = False
        if save_all_eval_results:
            with open("kitti_eval_info.pkl", "wb") as f:
                data = {
    ...
    def evaluation_with_custom_annos(
        self,
        det_annos,
        gt_annos,
        class_names,
        **kwargs,
        ...
        """
        Only used for copy-paste experiments. gt_annos are hard-coded to have
        "Car" type.
        """
        if "annos" not in self.kitti_infos[0].keys():
            return None, {}
        ...
        if len(gt_annos) != len(eval_gt_annos):
            raise ValueError(
                "gt_annos and eval_gt_annos have different lengths: "
                f"{len(gt_annos)} vs {len(eval_gt_annos)}"
            )

        # Replace specific values of "Car" in eval_gt_annos.
        # This needs to be very precisely.
        # - "name"            : No change
        # - "truncated"       : No change
        # - "occluded"        : No change
        # - "alpha"           : Masked update with gt_annos # slight diff (due to pixel precision?)
        # - "bbox"            : Masked update with gt_annos # slight diff (due to pixel precision?)
        # - "dimensions"      : Masked update with gt_annos # same
        # - "location"        : Masked update with gt_annos # same
        # - "rotation_y"      : Masked update with gt_annos # same
        # - "score"           : No change
        # - "difficulty"      : No change
        # - "index"           : No change
        # - "gt_boxes_lidar"  : Double masked ("Car" and "DontCare") # same
        # - "num_points_in_gt": No change
        for i in range(len(eval_gt_annos)):
            eval_mask = eval_gt_annos[i]["name"] == "Car"

            docare_names = eval_gt_annos[i]["name"][
                eval_gt_annos[i]["name"] != "DontCare"
            ]
            docare_and_eval_mask = docare_names == "Car"

            if len(eval_gt_annos[i]["alpha"][eval_mask]) != len(gt_annos[i]["alpha"]):
                print(
                    f"Index {i} has different number of gt boxes, where "
                    f"len(eval_gt_annos[i]['alpha'][eval_mask]) = "
                    f"{len(eval_gt_annos[i]['alpha'][eval_mask])} and "
                    f"len(gt_annos[i]['alpha']) = {len(gt_annos[i]['alpha'])}"
                )
                import ipdb
            ...
            eval_gt_annos[i]["alpha"][eval_mask] = gt_annos[i]["alpha"]
            eval_gt_annos[i]["bbox"][eval_mask] = gt_annos[i]["bbox"]
            eval_gt_annos[i]["dimensions"][eval_mask] = gt_annos[i]["dimensions"]
            eval_gt_annos[i]["location"][eval_mask] = gt_annos[i]["location"]
            eval_gt_annos[i]["rotation_y"][eval_mask] = gt_annos[i]["rotation_y"]
            eval_gt_annos[i]["gt_boxes_lidar"][docare_and_eval_mask] = gt_annos[i][
                "boxes_lidar"
            ][:, :7]
    ...
    def __getitem__(self, index):
        ...
        if "annos" in info:
            ...
            if self.dataset_cfg.get("REMOVE_ORIGIN_GTS", None) and self.training:
                input_dict["points"] = box_utils.remove_points_in_boxes3d(
                    input_dict["points"], input_dict["gt_boxes"]
                )
